	  ...
	  <div class="tabbed-content">
        ...
        <div class="tab_container">
		  ...
		  <?php foreach($events as $event) { ?>
			  <!-- Begin Toggle -->
			  <div class="toggle">
				<h2 class="trigger" onclick="$.loadEventInfo('<?php echo $event->{'userId'} ?>', '<?php echo $event->{'prepayment'} ?>', '<?php echo $event->{'status'} ?>', '<?php echo $event->{'id'} ?>', '<?php echo $event->{'entity'} ?>')"> + <?php echo $event->{'entityName'} ?>  - <?php echo $event->{'start_date'} ?> - <?php echo constant('LABEL'.$event->{'status'}) ?> </h2>
